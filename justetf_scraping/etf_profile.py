...
def get_gettex_quote(isin: str, currency: str = "EUR", language: str = "en") -> Optional[GettexQuote]:
    """
...
def get_gettex_quote_raw(isin: str, currency: str = "EUR", language: str = "en") -> Optional[Dict[str, Any]]:
    """
...
def _fetch_ajax_data(session: requests.Session, isin: str, endpoint: str, element_id: str) -> Optional[str]:
    """
...
def _parse_allocation_from_ajax(xml_response: str, table_id: str, name_testid: str, pct_testid: str) -> List[AllocationItem]:
    """
...
def _parse_allocation_from_soup(soup: BeautifulSoup, row_testid: str, name_testid: str, pct_testid: str) -> List[AllocationItem]:
    """
...
def get_etf_overview(isin: str, include_gettex: bool = True, expand_allocations: bool = True) -> EtfOverview:
    """
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Failed to fetch ETF page for {isin}: status {response.status_code}")
    ...
    top_holdings = []
    holdings_rows = soup.find_all('tr', attrs={'data-testid': 'etf-holdings_top-holdings_row'})
    ...
    if expand_allocations:
        ...
        countries_xml = _fetch_ajax_data(session, isin, 'holdingsSection-countries-loadMoreCountries', 'id6')
        if countries_xml:
            ...
            countries = _parse_allocation_from_soup(
                soup, 'etf-holdings_countries_row',
                'tl_etf-holdings_countries_value_name',
                'tl_etf-holdings_countries_value_percentage'
            )
    else:
    ...
    if expand_allocations:
        # Try AJAX for full list
        sectors_xml = _fetch_ajax_data(session, isin, 'holdingsSection-sectors-loadMoreSectors', 'id7')
        if sectors_xml:
